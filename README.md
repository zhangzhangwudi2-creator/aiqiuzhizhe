...
- 下载真正的 PDF 简历：自动提取原 PDF 中的证件照并固定放回页首；未检测到照片时拒绝导出，避免静默生成无照片版本。导出优先使用现代双栏简历模板（顶部信息区、贯穿页面的浅色信息栏、简历风格 section 标题、深色正文），长链接自动缩短展示，内容较长或布局异常时自动降级为稳定单栏模板；仍是轻量模板，不是商业模板库
